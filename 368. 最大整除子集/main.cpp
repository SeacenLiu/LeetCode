...
/**
 ...
 - 动态规划
    - 对数组由大到小排序
        - 如果满足nums[i]%nums[i+1]==0，那么nums[i+1]一定可以被能整除nums[i]的所有元素整除
    - 推导最大子集数
        - dp是递推数组，dp[i]表示到第i个元素最大的子集元素个数
        - 如果满足nums[i]%nums[i+1]==0并且dp[i]+1>=dp[i+1]的条件，就更新dp[i+1]=dp[i]+1
    - 保存最大子集数组
        - 使用 parent 数组表示有向图结构
        - parent[k]=q 的意思是记录第k个元素的最大子集开始的下标是q
        - 如果满足nums[i]%nums[i+1]==0并且dp[i]+1>=dp[i+1]的条件，就更新parent[i+1]=i
 */
...
private:
    vector<int> res;
    bool check(int add, vector<int> cur) {
        for (int  i = 0; i < cur.size(); ++i) {
            if (add % cur[i] != 0 && cur[i] % add != 0)
                return false;
        }
        return true;
    }
